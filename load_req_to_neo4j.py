#!/usr/bin/env python3
"""
Script to load a single requirement and its related code nodes into Neo4j for visualization.
"""
import os
import json
import sys
...
def load_requirement_and_related_files(req_id, analysis_file):
    """
    Load a requirement and its related code files into Neo4j for visualization.
    ...
    print("   MATCH (r:Requirement)-[rel:TRACED_TO]->(n:AdaNode) RETURN r, rel, n ORDER BY rel.score DESC")
    ...
    print("   MATCH p=(n:AdaNode)-[:CONTAINS*]->(c) RETURN p LIMIT 100")
    print("5. View code references:")
    print("   MATCH p=(n:AdaNode)-[:REFERENCES]->(c) RETURN p LIMIT 100")
    
if __name__ == "__main__":
    if len(sys.argv) < 2:
        req_id = "AUTH-01"  # Default requirement ID
        analysis_file = os.path.join("outputs", f"req_{req_id}_analysis.json")
    else:
        req_id = sys.argv[1]
    ...
    load_requirement_and_related_files(req_id, analysis_file) 